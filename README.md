...
| 分頁 | 內容 |
| --- | --- |
| **寫日記** | 日期選擇器與當日狀態、「三件事」、其他 (Notes)、這一天的共享內容，底部是暫存／送出；送出後會換成當天的完成畫面 |
| **回顧** | 回顧月曆、點選日期後的解密內容、資料匯出與匯入 |
| **設定** | 共用解密密碼、兩人共享（Workspace 與邀請碼），以及「進階」分組裡的加密字串與單筆還原 |
...
| 送出後的完成畫面 | 送出之後輸入欄位會收起來，換成一張安靜的完成卡片（打勾、當天的三件事與 Notes、加密時間與同步狀態），底部只剩一顆「修改這一天」，另附「看看過去寫過的」直接跳到回顧分頁。 |
| 加密字串與單筆還原（進階） | 設定分頁最後、獨立在「進階」分組裡，預設收起。顯示這一天的加密字串（可複製），也可以貼上備份檔或另一台裝置的加密字串，用密碼解回內容並「填回表單」。 |
...
| 資料匯入（還原備份） | 把備份檔整份讀回來。加密備份用密碼解開，明文備份用現在的密碼重新加密，兩者最後都以 envelope 回到 app。撞到現有內容時讓使用者選「保留現在的」（預設）或「用備份覆蓋」（需二次確認）；有 Workspace 時可一併同步上去。 |

其他細節：

- 每一天的內容各自保存在 `Record<dateKey, DayEntry>` 中，切換日期不會互相覆蓋，切回來內容還在。
- 「這一天的共享內容」與「回顧」的日期內容共用 `EntryList`，兩邊的呈現一定一致。
- 設定分頁分成「必要」與「進階」兩段：密碼與共享是每個人都要碰的，
  加密字串面板則預設收起（也不會在每次加密後自己展開），避免技術細節搶走注意力。
- 寫日記是一件有結束的事：送出之後不再把空白輸入框留在畫面上等人繼續打字，
  而是收成完成卡片，標題也從「今天有什麼值得感謝？」換成「今天已經寫完了」。
  要補寫再按「修改這一天」，改完重新送出就會再收起來。
- 完成卡片上的同步狀態如實顯示：真的上傳成功才寫「已同步」，
  上傳失敗是「尚未同步」，沒設定 Supabase 則是「只在這台裝置」。
- 已送出的那一天若再被修改，狀態會自動退回「已暫存」，提醒使用者重新送出。
- 匯出與匯入放在同一個分頁：只能匯出的話，「資料自主權」只做了一半——
  拿得回檔案，卻拿不回日記。還原一整年不該是 365 次複製貼上。
- 匯入只收檔案裡標記為自己的紀錄。另一半寫的會如實回報筆數但不匯入——
  那是對方的紀錄，不能改掛在我名下（匯入後送出會用我的標籤上傳）。
- 衝突偵測不只看目前這個頁面：還原是 upsert，會蓋掉 Workspace 上那一天，
  所以會先問一次遠端有哪幾天是自己寫的。
- 單筆壞掉（日期格式不對、ciphertext 不是 envelope）只跳過那一筆並回報筆數，
  不會讓整份備份都救不回來。
...
│   ├── CipherPanel.tsx      # 進階：加密字串顯示 + 單筆還原 + 填回表單
...
│   ├── ImportPanel.tsx      # JSON 匯入（還原備份）
│   ├── ActionBar.tsx        # 暫存 / 送出（都會先加密）／完成後的「修改這一天」
│   ├── DayCompleteCard.tsx  # 送出後取代編輯表單的完成畫面
